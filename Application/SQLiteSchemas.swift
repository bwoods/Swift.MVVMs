...
extension SQLiteWindow {
	func schemaVersioning() throws {
		...
		if sqlite3_prepare_v2(db, "PRAGMA user_version", -1, &stmt, nil) != SQLITE_OK { assertionFailure(String(cString: sqlite3_errmsg(db))); }
		defer { sqlite3_finalize(stmt) }

		if sqlite3_step(stmt) == SQLITE_ROW {
			var version: Int = numericCast(sqlite3_column_int(stmt, 0))
			if NSDataAsset(name: "v\(version)") == nil {
			...
			for n in version... {
				if let asset = NSDataAsset(name: "v\(n)") {
					if sqlite3_exec(db, String(data: asset.data, encoding: .utf8)!, nil, nil, nil) != SQLITE_OK { assertionFailure(String(cString: sqlite3_errmsg(db))) }
				} else {
					break
				}
			}
